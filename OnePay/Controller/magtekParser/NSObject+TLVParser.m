...
- (NSMutableDictionary*) parseTLVDataWithNoLength
{
    int len = self.length;
    Byte lengthByte[2] = {0x00};
    lengthByte[1] = (Byte)len;
    lengthByte[0] = (Byte)(len>>8);
    NSMutableData* tempData = [[NSMutableData alloc] init];
    [tempData appendBytes:&lengthByte[0] length:1];
    [tempData appendBytes:&lengthByte[1] length:1];
    [tempData appendData:self];
