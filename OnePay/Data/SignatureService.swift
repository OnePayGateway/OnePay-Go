...
class SignatureService: BaseRequest {
    
    var url: String!
    var params = [String:Any]()
    
    func sendSignature(signature:Signature, Oncomplete: @escaping(JSON?, Error?) -> Void) {
        
        let payment = Payment()
        let additionalData: [[String:Any]] = [payment.sourceDic, signature.signatureDic]
        
        let parameters = ["method":"CC", "type":signature.type_code, "action_code":signature.action_code, "nonce":Date().generateCurrentTimeStampAsNonce(), "reference_transaction_id":signature.ref_tran_id!, "additional_data":additionalData] as [String : Any]
